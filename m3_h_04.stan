data {
  ...
  array[NS, MT] int stickiness_indicator; // 1 = previous choice left, -1 = previous choice right
}
...
parameters {
  ...
  vector[NS] inv_temp_raw; // subject-level inverse temperature parameter
  vector[NS] stickiness_raw; // subject-level stickiness parameter
...
transformed parameters {
  ...
  for (s in 1:NS) {
    ...
    inv_temp[s] = exp(mu_inv_temp + sigma_inv_temp * inv_temp_raw[s]);
    stickiness_param[s] = mu_stickiness + sigma_stickiness * stickiness_raw[s];
  }
...
model {
  ...
  sigma_alpha_logit ~ normal(0, 1);
  mu_beta ~ normal(0, 1);
  sigma_beta ~ normal(0, 1);
  mu_inv_temp ~ normal(1, 0.5);
  sigma_inv_temp ~ normal(0, 1);
  mu_stickiness ~ normal(0, 1);
  sigma_stickiness ~ normal(0, 1);
    ...
    for (t in 1:NT[s]) {
      if (choice[s,t] > 0) {
        choice_two[s,t] ~ bernoulli_logit(inv_temp[s] * (beta[s,1] +
                                          ...
                                          stickiness_param[s] * stickiness_indicator[s,t]));
      }
    }
  }
}
