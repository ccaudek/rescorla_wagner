...
transformed parameters {
  ...
  // need to assign Q and PE because missing trials will recreate NaNs otherwise
  for (s in 1:NS) {
    for (m in 1:MT) {
      for (c in 1:NC) {
        Q[s,m,c] = 0.0;
  ...
  for (s in 1:NS) {
    for (t in 1:NT[s]) {
      ...
      if (t == 1) {
        for (c in 1:NC) {
          Q[s,t,c] = 0.5;  // want to change to 1/NC if Stan ever allows int_to_real
        }
        delta[s,t] = 0;
      }
      if (choice[s,t] >= 0) {
        ...
        if (t < NT[s]) {
          ...
          Q[s,t+1,abs(choice[s,t]-1)] = Q[s,t,abs(choice[s,t]-1)];
        }
      } else {
